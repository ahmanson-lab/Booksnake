...
struct MultiProperty {
    ...
    init?(_ json: Any?) {
        ...
        if let value = json as? String {
            // single value
            singleValue = value.trimmingCharacters(in: .whitespacesAndNewlines)
        } else if let array = json as? [String] {
            // simple array containing only Element
            arrayValue.append(contentsOf: array.map({ $0.trimmingCharacters(in: .whitespacesAndNewlines) }))
            ...
            for item in array {
                if let value = item as? String {
                    arrayValue.append(value.trimmingCharacters(in: .whitespacesAndNewlines))
    ...
    fileprivate mutating func parse(dictionary dict: [String:Any]) {
        if let lang = dict["@language"] as? String, let value = dict["@value"] as? String {
            dictValue[lang] = value.trimmingCharacters(in: .whitespacesAndNewlines)
        }
        for (key, val) in dict where key != "@language" && key != "@value"{
            dictValue[key] = val is String ? (val as! String).trimmingCharacters(in: .whitespacesAndNewlines) : val
        }
    }
