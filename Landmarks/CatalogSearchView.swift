	...
	var body: some View {
			...
			ZStack(alignment: .top, content: {
				...
				VStack{
					...
					NavigationLink(
						destination: LazyView(FullWebView(hasJP2: $hasJP2, label: $label, filter: filter, type: type, delegate: delegate, webview: WebViewRepresentable(search: fieldURL.joined(separator: fieldValue), path: $urlPath, isJP2: $hasJP2, filter: filter, hasBackList: $hasBackList, hasForwardList: $hasForwardList, viewModel: model))),
                        isActive: $active,
						label: {
							ZStack(){
